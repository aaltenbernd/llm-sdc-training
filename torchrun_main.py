...
def parse_args(args):
    ...
    parser.add_argument("--fi_nvbit_alpha", type=float, default=0.05)
...
def main(args):
    ...
    rt_jump_history = torch.zeros(args.warmup_steps, device=device)
    gradient_norm_jump = torch.tensor(0.0, device=device)

    gradient_norm_pre_prev = torch.tensor(0.0, device=device)
    ...
    confusion_count = {
        ...
        "tn": 0,
        "fn": 0,
        "count_correct_recompute": 0, 
        "count_incorrect_recompute": 0
    }

    window_mean = torch.tensor(0.0, device=device)
    ...
    for batch_idx, batch in enumerate(dataloader, start=start_batch_idx):
        ...
        if global_step < args.warmup_steps:
            rt_jump_history[global_step] = rt_jump.detach()
        ...
        while do_recompute:
            ...
            anomaly = False
            warning = None
            if not torch.isfinite(gradient_norm_pre):
                anomaly = True
                warning = f"gradient_norm_pre not finite"
            else:
                if global_step >= 10:
                    gradient_norm_jump = gradient_norm_pre - gradient_norm_pre_prev
                    gradient_norm_jump = torch.clamp_min(gradient_norm_jump, 0)

                    if gradient_norm_pre_prev.eq(0):
                        anomaly = False
                    elif gradient_norm_jump.eq(0):
                        anomaly = False
                    else:
                        if global_step < args.warmup_steps:
                        ...
                        threshold = window_mean / (args.fi_nvbit_alpha * gradient_norm_jump_sqrt)
                        anomaly = rt_jump > threshold

                        if anomaly:
                            warning = (
                                f"{rt_jump.item():.4e} > "
                                f"{window_mean.item():.4e} / "
                                f"({args.fi_nvbit_alpha} * {gradient_norm_jump_sqrt.item():.4e})"
                            )

            anomaly = bool(anomaly)
            if rt_jump_prev is not None:
                if anomaly:
                    confusion_count['count_incorrect_recompute'] += 1
                    logger.warning(f"[AD] Anomaly persists after recomputation")
                else:
                    confusion_count['count_correct_recompute'] += 1
                    logger.warning(f"[AD] Resolved after recomputation")

                do_recompute = False
                rt_jump_prev = None
            else:
                if anomaly:
                    logger.warning(f"[AD] Detected anomaly {global_step}: {warning}")
                    if trigger_nvbit:
                        confusion_count['tp'] += 1
                    else:
                        confusion_count['fp'] += 1
                else:
                    if trigger_nvbit:
                        confusion_count['fn'] += 1
                    else:
                        confusion_count['tn'] += 1

                anomaly_log = 1.0 if anomaly else 0.0

                if args.fi_nvbit_recompute and anomaly:
                    trigger_nvbit = False
                    do_recompute = True
                    rt_jump_prev = rt_jump.detach()
                    optimizer.zero_grad()

                    logger.warning(f"[AD] Doing recompute")
                else:
                    do_recompute = False
                    rt_jump_prev = None
        ...
        gradient_norm_pre_prev = gradient_norm_pre
        ...
        if global_rank == 0:
            wandb.log({
                    ...
                    "update_step": update_step, 
                    "gradient_norm_jump": gradient_norm_jump,
                    "detected_anomaly": anomaly_log,
                    "trigger_nvbit": trigger_nvibit_log,
                },
                step=global_step,
            )
            wandb.log(rt_dict, step=global_step)
    ...
    if global_rank == 0:
        wandb.summary["confusion_count"] = confusion_count
        wandb.finish()
